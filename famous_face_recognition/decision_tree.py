...
# function to create a required dataset by training data: face encodings and names
def read_face(encodings, names, classifier=CLASS_NAME):

    dataset = data(classifier)

    for i in range(len(encodings)):
        face_list = encodings[i].tolist() + [names[i]]
        dataset.examples.append(face_list)

    dataset.attributes = [ "a"+str(i) for i in range(128) ]
    dataset.attributes.append(classifier)
    dataset.attr_types = [ True for i in range(128) ]
    dataset.attr_types.append(False)
    dataset.class_index = 128
...
def train(encodings, names):
    dataset = read_face(encodings, names)
    ...
    root = compute_tree(dataset, None, CLASS_NAME) 
    print_tree(root)
    return root


def predict(encoding, tree):
    #TODO: currently no solve two image problem
    testcase = encoding[0].tolist()
    testcase.append("?")
...
'''
# currently do not consider these feature
#        if ("-s" in args):
...
# TODO: move test part to predict function
#        if ("-t" in args):
